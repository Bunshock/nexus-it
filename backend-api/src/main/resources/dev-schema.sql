...
-- (dev) — and the desktop REST client that develops against it — has a real, populated database
...
-- is a hand-maintained mirror, table set + columns matching V1/V2, in the M1 (GLPI-adapter)
-- stripped shape — ported from v1 (no-adapter)'s own dev-schema.sql and cut down to match:
-- MODEL_STOCK/SN_VALIDATION/SEDE_SHIPPING_INFO/NOTE_ITEM_STOCK_EXCEPTION/SMTP columns removed,
-- NOTE_REMITO_SEDE/NOTE_REMITO_OTHER collapsed into one NOTE_REMITO(destination_sede_id),
-- NOTE_ITEM.modifies_stock/NOTE_REPORT.stock_applied dropped, NOTE_ITEM.external_item_id and
-- NOTE_ITEM_RETURN_ALLOCATION.external_item_id added. Identity + datetime types use the
-- MSSQL-compatible forms H2's MODE=MSSQLServer accepts (AUTO_INCREMENT / TIMESTAMP do not parse
-- under that mode). Keep it in sync when V1/V2 change.
--
-- Every statement is IF NOT EXISTS so a devtools restart (H2 mem can survive one) re-runs harmlessly.
-- Filtered unique indexes (idx_model_global_generic_name / idx_model_single_active_generic in V1)
-- are deliberately omitted — H2 has no partial-index support; the dev seed just inserts one
-- global-generic MODEL row.
...
-- ============================================================================
-- Equipment catalog — read-only from the app as of M1 (see CatalogController)
-- ============================================================================
...
-- ============================================================================
-- Config (V2__config.sql equivalent) — SMTP columns removed (M1)
-- ============================================================================
...
-- ============================================================================
-- Audit trail (append-only). AUDIT_STOCK is left in place even though nothing writes to it any
-- more post-M1-strip — see V1__init.sql's own comment on the same open question.
-- ============================================================================
...
-- M1: collapsed from NOTE_REMITO_SEDE/NOTE_REMITO_OTHER into a single mandatory destination Sede.
CREATE TABLE IF NOT EXISTS NOTE_REMITO (
    note_report_id INT PRIMARY KEY REFERENCES NOTE_REPORT(id),
    destination_sede_id INT NOT NULL REFERENCES SEDE(id)
);

-- external_item_id (M1, added for M4 to populate): the eventual GLPI asset id this item resolves to.
...
CREATE TABLE IF NOT EXISTS AUDIT_ITEM_STATUS (
    id          INT IDENTITY(1,1) PRIMARY KEY,
    item_id     INT NOT NULL REFERENCES NOTE_ITEM(id),
    status_kind VARCHAR(20) NOT NULL CHECK (status_kind IN ('GLPI', 'RETURN')),
...
-- external_item_id (M1, added for M5b to populate): which specific GLPI Peripheral row this
-- allocation's units resolved to, once the write-queue worker picks them.
