...
- **`dni` can be a JSON array**: discovered 2026-07-08 via a real crash (`MismatchedInputException`) on a broad search — some accounts return `"dni"` as a multi-valued array instead of a plain string. `AdApiUserDto.dni` is typed `JsonNode` (not `String`) and normalized via `extractDni()`: plain string → as-is, array → first element (or `""` if empty), `null`/missing → `null`. If any other field on this DTO turns out to have the same multi-valued shape, apply the same `JsonNode` + extractor pattern rather than assuming every field is a plain string.
