...
// Loads EnviosView.fxml (the top-level Envíos toggle shell, mirroring PrestamosView.fxml's own
// wrapper shape) through a real FXMLLoader — no other test does, so a typo in its fx:id/onAction
// would otherwise only surface at runtime.
class EnviosViewFxmlTest {
    ...
    void loadsWithoutFxmlBindingErrors() throws Exception {
        ...
        Platform.runLater(() -> {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource(
                    "/com/bunshock/note_app_for_it_frontend/views/EnviosView.fxml"));
        ...
        if (errorRef.get() != null) {
            fail("EnviosView.fxml failed to load: " + errorRef.get(), errorRef.get());
        }
