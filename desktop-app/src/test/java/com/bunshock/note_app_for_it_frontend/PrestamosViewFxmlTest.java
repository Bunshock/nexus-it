...
// Loads PrestamosView.fxml (the top-level Préstamos toggle shell) through a real FXMLLoader —
// no other test does, so a typo in its fx:id/onAction would otherwise only surface at runtime.
class PrestamosViewFxmlTest {
    ...
    void loadsWithoutFxmlBindingErrors() throws Exception {
        ...
        Platform.runLater(() -> {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource(
                    "/com/bunshock/note_app_for_it_frontend/views/PrestamosView.fxml"));
        ...
        if (errorRef.get() != null) {
            fail("PrestamosView.fxml failed to load: " + errorRef.get(), errorRef.get());
        }
