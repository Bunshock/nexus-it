...
// Loads NoteTabContentView.fxml through a real FXMLLoader — no other test does, so a typo in its
// fx:id/onAction would otherwise only surface at runtime. NoteTabController.initialize() touches
// no ServiceLocator/DB state, making a full FXML load cheap and realistic — same reasoning
// NoteGeneratorViewFxmlTest relied on before this content moved out of the tab-bar shell.
class NoteTabContentViewFxmlTest {
        ...
        Platform.runLater(() -> {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource(
                    "/com/bunshock/note_app_for_it_frontend/views/NoteTabContentView.fxml"));
                loader.load();
                NoteTabController controller = loader.getController();
                Field f = NoteTabController.class.getDeclaredField("txtObservations");
    ...
    void noteTypeComboBoxListsAllFiveTypesAndObservationsFooterResolves() throws Exception {
        ...
        Platform.runLater(() -> {
            try {
                ...
                Field comboField = NoteTabController.class.getDeclaredField("cmbNoteType");
                ...
                Field footerField = NoteTabController.class.getDeclaredField("vboxObservationsFooter");
