...
// Loads RemitoTabContentView.fxml through a real FXMLLoader — catches fx:id/onAction typos that
// would otherwise only surface at runtime. initialize() dereferences equipmentService
// synchronously (populateDestinationSedeCombo()), same as RemitoNoteViewFxmlTest did before this
// content moved out of the tab-bar shell, so ServiceLocator needs a real service before load.
class RemitoTabContentViewFxmlTest {
    ...
    void loadsWithoutFxmlBindingErrors() throws Exception {
        ...
        if (errorRef.get() != null) {
            fail("RemitoTabContentView.fxml failed to load: " + errorRef.get(), errorRef.get());
        }
        ...
        Platform.runLater(() -> {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource(
                    "/com/bunshock/note_app_for_it_frontend/views/RemitoTabContentView.fxml"));
                loader.load();
                RemitoTabController controller = loader.getController();
                Field f = RemitoTabController.class.getDeclaredField("txtDestinationLabel");
    ...
    void observationsFieldRejectsInputBeyond300Characters() throws Exception {
        ...
        Platform.runLater(() -> {
            try {
                ...
                loader.load();
                RemitoTabController controller = loader.getController();
                Field f = RemitoTabController.class.getDeclaredField("txtObservations");
                f.setAccessible(true);
                TextField txt = (TextField) f.get(controller);
                fieldRef.set(txt);
