...
// Mirrors DatabaseSectionViewFxmlTest — catches fx:id/onAction typos. initialize() reads
// ConfigService.getConfig().fallaOptions synchronously, so config must be loaded first.
class FailureDetailViewFxmlTest {
    ...
    void loadsWithoutFxmlBindingErrors() throws Exception {
        ...
        if (errorRef.get() != null) {
            fail("FailureDetailView.fxml failed to load: " + errorRef.get(), errorRef.get());
        }
