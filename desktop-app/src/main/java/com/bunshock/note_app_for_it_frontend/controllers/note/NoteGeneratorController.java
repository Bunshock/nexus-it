...
// Tab-bar shell for "Generar Nota" — owns the open-page list, delegates per-page logic to
// NoteTabController (loaded fresh, never cached). Excel-style: pageStack shows the active page,
// pageTabsBox is a plain ToggleButton row (not a real TabPane — that couples header/content width).
public class NoteGeneratorController {
    ...
    private final Tooltip maxTabsTooltip = new Tooltip("Se alcanzó el máximo de " + MAX_NOTE_TABS + " notas.");
    ...
    private record NotePage(ToggleButton toggle, Node content, NoteTabController controller, Label nameLabel) {}

    private final ToggleGroup pageToggleGroup = new ToggleGroup();
    private final List<NotePage> pages = new ArrayList<>();
    ...
    private NotePage createNotePage() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource(
                "/com/bunshock/note_app_for_it_frontend/views/NoteTabContentView.fxml"));
            Parent content = loader.load();
            NoteTabController ctrl = loader.getController();
            ...
            NotePage page = new NotePage(toggle, content, ctrl, nameLabel);
            ...
            ctrl.setTabDisplayChangeListener(this::refreshTabChrome);
            ...
            throw new RuntimeException("Failed to load NoteTabContentView.fxml", e);
        }
    }
    ...
    private void closePage(NotePage page) {
    ...
    private void refreshTabChrome() {
        ...
        for (NotePage p : pages) {
            ...
            p.toggle().setText(name); // not rendered (GRAPHIC_ONLY) — kept for accessibility tools
        ...
        Platform.runLater(() -> {
            for (NotePage p : pages) updateTabTooltip(p);
        });
    }
    ...
    private void updateTabTooltip(NotePage page) {
        String text = page.controller().getTabDisplayName();
    ...
    private void handleGenerateNote() {
        NotePage active = activePage();
        if (active != null) active.controller().generateNote();
    }
    ...
    // Called by MainController on every navigation into this section — delegates to whichever
