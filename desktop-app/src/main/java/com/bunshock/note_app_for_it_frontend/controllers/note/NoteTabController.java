...
// One instance per open "Generar Nota" tab — loaded fresh via FXMLLoader by
// NoteGeneratorController (the tab-bar shell), never cached via ViewFactory, same "load fresh
// every time" precedent this codebase already uses for ItemDialogController. Holds everything
// that used to be single-instance on NoteGeneratorController itself: item lists, the ItemDialogHost
// implementation, observations field, stock-warning, and its own UserNoteController/
// ProviderNoteController pair (also loaded fresh per tab, not through ViewFactory).
public class NoteTabController implements ItemDialogHost {
    ...
    // Set by NoteGeneratorController right after this tab's content is loaded — lets the
    // "close tab after generation" preference (see openPreview()'s success callback) close
    // this specific tab without this class needing any reference to the TabPane/shell itself.
    private Runnable closeTabRequest;

    public void setCloseTabRequest(Runnable closeTabRequest) {
        this.closeTabRequest = closeTabRequest;
    }

    // Set by NoteGeneratorController right after this tab's content is loaded, alongside
    // setCloseTabRequest() — lets the shell's tab label (see getTabDisplayName()) stay in sync
    // with this tab's own note-type selection, provider pick, or recipient name, without this
    // class needing a reference back to the TabPane/shell itself. Renamed from
    // "noteTypeChangeRequest" once this started covering more than just the type combobox.
    private Runnable tabDisplayChangeRequest;

    public void setTabDisplayChangeListener(Runnable tabDisplayChangeRequest) {
        this.tabDisplayChangeRequest = tabDisplayChangeRequest;
    }

    private void notifyTabDisplayChanged() {
        if (tabDisplayChangeRequest != null) tabDisplayChangeRequest.run();
    }

    // Success-message label lives on the shell's shared bottom bar now, not per-tab.
    private Consumer<String> statusMessageRequest;

    public void setStatusMessageListener(Consumer<String> statusMessageRequest) {
        this.statusMessageRequest = statusMessageRequest;
    }

    // Read by NoteGeneratorController.refreshTabChrome() — "<name> - <tipo>" once a real name
    // (the picked provider, or the recipient's AD/manually-entered name) is available, or just
    // the plain note type alone until then — the type itself is never dropped, unlike an earlier
    // version of this method that replaced it outright once a name showed up. Never blank:
    // NOTE_TYPE_OPTIONS always has a selection by the time this is called
    // (cmbNoteType.getSelectionModel().selectFirst() runs at the end of initialize()).
    public String getTabDisplayName() {
        NoteTypeOption selected = cmbNoteType.getValue();
        if (selected == null) return "";
        String name = selected.isProvider()
            ? getProviderNoteController().getProviderName()
            : getUserNoteController().getTabDisplayName();
    ...
    public void initialize() {
        ...
        cmbNoteType.valueProperty().addListener((obs, old, selected) -> {
            ...
            if (selected.isProvider()) {
                ...
                showUserNoteView();
                getUserNoteController().setNoteType(selected.rawType());
            }
            refreshStockWarning();
            notifyTabDisplayChanged();
        });
        ...
        // Fires the cmbNoteType listener above, landing on the first option ("Entrega") — was
        // deferred to setViewFactory(...) on NoteGeneratorController; this tab needs no such
        // external hook, so it happens directly at the end of initialize().
    ...
    // Loaded fresh, once per tab, and cached only on this instance — deliberately NOT routed
    // through ViewFactory (which would make it a single shared instance across every open tab).
    private VBox getUserNoteView() {
        if (userNoteView == null) {
            try {
                ...
                userNoteController.setTabNameChangeListener(this::notifyTabDisplayChanged);
            } catch (IOException e) {
                throw new RuntimeException("Failed to load UserNoteView.fxml", e);
            }
        }
        return userNoteView;
    }

    private VBox getProviderNoteView() {
        if (providerNoteView == null) {
            try {
                ...
                providerNoteController.setTabNameChangeListener(this::notifyTabDisplayChanged);
            } catch (IOException e) {
                throw new RuntimeException("Failed to load ProviderNoteView.fxml", e);
            }
        }
        return providerNoteView;
    }

    private UserNoteController getUserNoteController() {
        getUserNoteView();
        return userNoteController;
    }

    private ProviderNoteController getProviderNoteController() {
        getProviderNoteView();
        return providerNoteController;
    }

    private void showUserNoteView() {
        dynamicContentArea.getChildren().setAll(getUserNoteView());
        setObservationsFooterVisible(true);
    }

    private void showProviderNoteView() {
        dynamicContentArea.getChildren().setAll(getProviderNoteView());
        // Refreshed on every switch to this tab's own Proveedor view (not a session-wide
        // ViewFactory cache anymore), so a provider added via Base de Datos mid-session still
        // shows up here the next time this tab's technician switches to Proveedor.
        getProviderNoteController().refreshProviders();
        setObservationsFooterVisible(true);
    }
    ...
    // Not @FXML-bound — the shell's shared "Limpiar Formulario" button delegates here.
    public void clearForm() {
        ...
        if (isUserNote() && userNoteController != null) {
            userNoteController.clearAllFields();
        } else if (!isUserNote() && providerNoteController != null) {
            providerNoteController.clearAllFields();
        }
    }
    ...
    private void showGenerationSuccess(String message) {
        if (statusMessageRequest != null) statusMessageRequest.accept(message);
    }

    // Not @FXML-bound — the shell's shared "Generar Nota" button delegates here.
    public void generateNote() {
        try {
            ...
            if (isUserNote() && !getUserNoteController().validateAndShowErrors()) {
                canGenerate = false;
            }
            if (!isUserNote() && !getProviderNoteController().validateAndShowErrors()) {
                canGenerate = false;
            }
            ...
            if (isUserNote()) {
                UserNoteController unc = getUserNoteController();
                ...
                ProviderNoteController pnc = getProviderNoteController();
    ...
    private boolean checkGlpiAssignments() {
        UserNoteController unc = userNoteController;
    ...
    private void openPreview(String html, String profileType, NoteReport report) throws IOException {
        ...
        ctrl.setOnSuccess(() -> {
            ...
            // Two independent per-technician preferences (Configuración → Generación de notas):
            // closing the tab takes priority over clearing it in place, since clearing a tab
            // that's about to be destroyed is moot.
            if (TechnicianSessionService.getInstance().isAutoCloseTabAfterGeneration() && closeTabRequest != null) {
                closeTabRequest.run();
            } else if (TechnicianSessionService.getInstance().isAutoClearFormAfterGeneration()) {
                clearForm();
            }
        });
