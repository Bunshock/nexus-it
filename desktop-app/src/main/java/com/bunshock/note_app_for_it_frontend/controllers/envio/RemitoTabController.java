...
// One instance per open "Remito de Envío" tab — loaded fresh via FXMLLoader by
// RemitoNoteController (the tab-bar shell), never cached via ViewFactory, same "load fresh every
// time" precedent this codebase already uses for ItemDialogController. Holds everything that used
// to be single-instance on RemitoNoteController itself: destination fields, item lists, the
// ItemDialogHost implementation, observations field, and stock-warning.
//
...
// one-off destination (e.g. a CAU not in the catalog). Unlike PrestamoTabController's no-print
// entry point, this one does render/print — it routes through the same NotePreviewController
// popup every other note type uses. Stock only actually moves once an admin
// approves the resulting note (see SqliteHistoryService.applyRemitoStockIfNeeded()).
public class RemitoTabController implements ItemDialogHost {
    ...
    // Set by RemitoNoteController right after this tab's content is loaded — lets the
    // "close tab after generation" preference (see openPreview()'s success callback) close this
    // specific tab without this class needing any reference to the TabPane/shell itself.
    ...
    // Set by RemitoNoteController right after this tab's content is loaded — notified whenever
    // this tab's display name (see getTabDisplayName()) might have changed.
    ...
    // Feeds RemitoNoteController's tab label — null/blank until a real destination name is
    // available, at which point the enclosing tab shows this instead of the plain "Remito"
    // fallback. Two sources, mirroring the same "trusted pick vs. manual entry" split
    // UserNoteController/PrestamoTabController use for a recipient name: a catalog Sede pick
    // (fillDestinationFields()) shows the Sede's own full name immediately — same as Provider's
    // combobox rule, no truncation, since a Sede name is a single identity, not a full person's
    // name; "Personalizar destino"'s own free-text "DESTINO" field instead shows just its first
    // word, and only once it loses focus — same manual-entry convention as "Nombre Completo".
    ...
    public void initialize() {
        ...
        // Only ever meaningfully fires in "Personalizar destino" mode — the field is disabled
        // (see below) whenever a catalog Sede drives it instead, and a disabled field can't
        // receive/lose focus from user interaction.
        txtDestinationLabel.focusedProperty().addListener((obs, wasFocused, isFocused) -> {
            if (!isFocused && chkCustomDestination.isSelected()) {
                tabDisplayName = firstWord(txtDestinationLabel.getText());
    ...
    // Not @FXML-bound — the shell's shared "Generar Remito" button delegates here.
    ...
    // ── Preview popup (duplicated from NoteTabController per this codebase's
