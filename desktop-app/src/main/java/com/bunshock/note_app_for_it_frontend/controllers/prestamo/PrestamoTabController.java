...
// One instance per open "Cargar Préstamo Interno" tab — loaded fresh via FXMLLoader by
// PrestamoNewLoanController (the tab-bar shell), never cached via ViewFactory, same "load fresh
// every time" precedent this codebase already uses for ItemDialogController/NoteTabController/
// RemitoTabController. A lightweight, non-printing sibling of NoteGeneratorController's Préstamo
// flow (see CLAUDE.md's "Préstamos section"). It logs a loan straight into history, with no HTML
// render/print/email step at all — that's the entire difference from the printed flow, which is
// left completely untouched.
public class PrestamoTabController implements ItemDialogHost, AdSearchHost {
    ...
    // Set by PrestamoNewLoanController right after this tab's content is loaded — lets the
    // "close tab after generation" preference (see guardarPrestamo()'s success handling) close
    // this specific tab without this class needing any reference to the shell itself. Same
    // precedent as NoteTabController/RemitoTabController.
    ...
    // Same duplicated AD-lookup(lastWord)/manual(firstWord-on-blur) logic as
    // UserNoteController.getTabDisplayName() — see that class for the full rationale.
    ...
    public void initialize() {
        ...
        // A real, user-driven edit (not fillUserData()'s own programmatic setText()) means any
        // AD-sourced name is no longer trustworthy as-is — the next blur recomputes from
        // whatever's actually in the field now, via firstWord().
    ...
    // Not @FXML-bound — the shell's shared "Guardar Préstamo" button delegates here.
    public void guardarPrestamo() {
        ...
        // closing the tab takes priority over clearing it in place, since clearing a tab that's
        // about to be destroyed is moot. Same shape as NoteTabController/RemitoTabController's
        // own post-generation success handling.
