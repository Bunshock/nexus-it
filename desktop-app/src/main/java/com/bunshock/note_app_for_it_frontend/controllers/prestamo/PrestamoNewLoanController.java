...
// Tab-bar shell for "Cargar Préstamo Interno" — mirrors NoteGeneratorController's/
// RemitoNoteController's shape exactly (see NoteGeneratorController's header comment for why
// this is a ToggleButton strip, not a real TabPane).
public class PrestamoNewLoanController {
    ...
    private final Tooltip maxTabsTooltip = new Tooltip("Se alcanzó el máximo de " + MAX_NOTE_TABS + " préstamos.");
    ...
    private record LoanPage(ToggleButton toggle, Node content, PrestamoTabController controller, Label nameLabel) {}

    private final ToggleGroup pageToggleGroup = new ToggleGroup();
    private final List<LoanPage> pages = new ArrayList<>();
    ...
    private LoanPage createLoanPage() {
        try {
            ...
            LoanPage page = new LoanPage(toggle, content, ctrl, nameLabel);
            ...
            throw new RuntimeException("Failed to load PrestamoTabContentView.fxml", e);
        }
    }

    private void selectPage(LoanPage page) {
        page.toggle().setSelected(true);
    }

    // At least 1 page must always stay open — replaces it with a fresh one if that was the last.
    private void closePage(LoanPage page) {
    ...
    private void refreshTabChrome() {
        ...
        for (LoanPage p : pages) {
            ...
            String display = (name != null && !name.isBlank()) ? name : "Préstamo";
        ...
        Platform.runLater(() -> {
            for (LoanPage p : pages) updateTabTooltip(p);
        });
    }
    ...
    private void updateTabTooltip(LoanPage page) {
    ...
    private void handleGuardarPrestamo() {
        LoanPage active = activePage();
        if (active != null) active.controller().guardarPrestamo();
    }
    ...
    // Called by PrestamosController on every navigation into this section — delegates to
    // whichever page is currently selected, since stock shortages are now a per-page concern.
